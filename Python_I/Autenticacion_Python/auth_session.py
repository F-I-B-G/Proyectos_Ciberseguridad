...
class AuthSession:
    ...
    def authenticate_and_fetch(self, target_url):
        ...
        if f'Welcome {self.username}'.lower() in response_login.text.lower():
            ...
            print(f'[+] Acceso autenticado a la URL realizado satisfactoriamente.') 
            return response # Se retorna el contenido de la página autenticada
        else:
            print('[!] Error en la autenticación. \nCódigo del estado de la respuesta: ', response_login.status_code)
            return response_login # Para saber el por qué

if __name__ == '__main__':
    base_url = input('Ingrese la URL BASE o presione ENTER para usar la predeterminada: ') or 'http://192.168.146.143:8080'
    username = 'usuario1'
    password = '1234'
    auth_session = AuthSession(base_url, username, password)

    # Especificar la URL que se quiere acceder de manera autenticada
    target_url = input('Ingrese la URL OBJETIVO o presione ENTER para usar la predeterminada: ') or 'http://192.168.146.143:8080/htmli_get.php'
